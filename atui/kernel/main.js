...
try {
    ...
    atuiKernel_Header.style.minHeight =
        atuiKernel_HeaderAside.clientHeight + "px";
    const atuiKernel_Carousel = atuiKernel_Header.childNodes[3];
    if (document.documentElement.clientWidth > 767) {
        const atuiKernel_CarouselHeight =
            atuiKernel_Carousel.clientHeight +
            atuiKernel_HeaderAside.clientHeight * 2;
        atuiKernel_Carousel.style.height = atuiKernel_CarouselHeight + "px";
    }
    atuiKernel_Carousel.style.paddingTop =
        atuiKernel_HeaderAside.clientHeight + "px";
} catch {}
...
try {
    const atuiKernel_ToolsSettingsDisplaymodeElement = document.getElementById(
        "atuiKernel_ToolsSettingsDisplaymode"
    ).childNodes[1];
    atuiKernel_ToolsSettingsDisplaymodeElement.addEventListener(
        "click",
...
function atuiKernel_ToolsSettingsDisplaymodeChange() {
    ...
    if (atuiKernel_ToolsSettingsDisplaymodeStatus) {
        ...
        atuiKernel_ToolsSettingsDisplaymodeElement.setAttribute(
            "src",
            "atui/kernel/medias/dark.png"
        );
        atuiKernel_ToolsSettingsDisplaymodeElement.setAttribute(
            "alt",
            "Mode sombre activé"
...
function atuiKernel_ColorschemeGenerator(base, isDarkMode, wantOpacity) {
    ...
    for (let tones = 1; tones < 5; tones++) {
        ...
        for (let primary = 0; primary < 3; primary++) {
            tone.push(
                Math.round(base[primary] + stepColor[primary + 3] * tones)
            );
        }
        schemeColor.push(tone);
    }
...
function atuiKernel_ColorschemeGeneratorPack(bicolor, accent, isDarkMode) {
    if (bicolor) {
        atuiKernel_ColorschemeGenerator(
            [127.5, 127.5, 127.5],
            isDarkMode,
            false
        );
...
function atuiKernel_ColorschemeGeneratorAuto(accent) {
    ...
    atuiKernel_ColorschemeGeneratorPack(
        true,
        defaultAccent,
...
try {
    const atuiKernel_FooterInfo = document.getElementById(
        "atuiKernel_FooterInfo"
    );
} catch {}
...
function atuiKernel_FooterLastedited(day, month, year) {
    ...
    atuiKernel_FooterInfoBased.setAttribute(
        "href",
        atuiKernel_Metadata["website"]
    );
    ...
    atuiKernel_FooterInfoLasteditedText.innerHTML =
        "Dernière modification le " + day + " " + month + " " + year + ". ";
    atuiKernel_FooterInfoLasteditedText.setAttribute(
        "href",
        "./atui/kernel/about.html"
    );
    atuiKernel_FooterInfoLasteditedText.setAttribute("target", "_blank");
    atuiKernel_FooterInfo.insertBefore(
        atuiKernel_FooterInfoLasteditedText,
        atuiKernel_FooterInfo.firstChild
    );
}
...
try {
    const atuiKernel_Notification = document.getElementById(
        "atuiKernel_Notification"
    );
} catch {}
...
function atuiKernel_NotificationDisplay(type, buttons, actions, title, text) {
    ...
    atuiKernel_NotificationElement.classList.add(
        "atuiKernel_NotificationElement"
    );
    ...
    // Type
    const atuiKernel_NotificationElementHeaderTypeImg =
        document.createElement("img");
    if (type != "normal") {
        atuiKernel_NotificationElementHeaderTypeImg.setAttribute(
            "src",
            "atui/kernel/medias/" + type + ".png"
        );
    } else {
    ...
    // Sound
    const atuiKernel_NotificationElementSound = new Audio(
        "atui/kernel/medias/notification.mp3"
    );

    // Title
    const atuiKernel_NotificationElementHeaderTitle =
        document.createElement("h3");
    atuiKernel_NotificationElementHeaderTitle.textContent = title;

    // Close button
    const atuiKernel_NotificationElementHeaderClose =
        document.createElement("img");
    atuiKernel_NotificationElementHeaderClose.setAttribute(
        "src",
        "atui/kernel/medias/close.png"
    );
    atuiKernel_NotificationElementHeaderClose.addEventListener(
        "click",
        function () {
    ...
    if (buttons != "insertion") {
        for (let counter = 0; counter < buttons.length; counter++) {
            const atuiKernel_NotificationElementFooterButton =
                document.createElement("button");
            atuiKernel_NotificationElementFooterButton.textContent =
                buttons[counter];
            atuiKernel_NotificationElementFooterButton.addEventListener(
                "click",
                function () {
                    atuiKernel_NotificationClose(
                        atuiKernel_NotificationElement
                    );
                    return actions[counter];
                }
            );
            atuiKernel_NotificationElementFooter.appendChild(
                atuiKernel_NotificationElementFooterButton
            );
        }
    } else {
        const atuiKernel_NotificationElementFooterInput =
            document.createElement("input");
        atuiKernel_NotificationElementFooterInput.setAttribute("type", "text");
    }

    // Insertion des éléments dans HTML
    atuiKernel_NotificationElementHeader.appendChild(
        atuiKernel_NotificationElementHeaderTypeImg
    );
    atuiKernel_NotificationElementHeader.appendChild(
        atuiKernel_NotificationElementHeaderTitle
    );
    atuiKernel_NotificationElementHeader.appendChild(
        atuiKernel_NotificationElementHeaderClose
    );
    atuiKernel_NotificationElement.appendChild(
        atuiKernel_NotificationElementHeader
    );
    atuiKernel_NotificationElement.appendChild(
        atuiKernel_NotificationElementText
    );
    atuiKernel_NotificationElement.appendChild(
        atuiKernel_NotificationElementFooter
    );
    ...
    if (!wish) {
        ...
        if (centered) {
            ...
            elementPositionY =
                ...
                20; /* Position top sur l'écran du récepteur */
            ...
            elementPositionX =
                ...
                20; /* Position left sur l'écran du récepteur */
            elementPositionY =
                element.getBoundingClientRect().y +
                20; /* Position top sur l'écran du récepteur */
            if (
                elementPositionX + cible.clientWidth >
...
function atuiKernel_TabsDisplay(element) {
    ...
    container
        .querySelector(".atuiKernel_SectionTabsHeader .active")
        .classList.remove("active"); // Remove active class to the activated tab
    element.classList.add("active"); // Add active class to clicked element
    container
        .querySelector(".atuiKernel_SectionTabsContent .active")
        .classList.remove("active"); // Remove active class to the activated content
    container
        .querySelector(element.getAttribute("href"))
        .classList.add("active"); // Add active element to content corresponding to clicked element
}
...
const atuiKernel_SectionAccordionContainer = document.getElementsByClassName(
    "atuiKernel_SectionAccordion"
);
let atuiKernel_SectionAccordionHeader;
for (let i = 0; i < atuiKernel_SectionAccordionContainer.length; i++) {
    atuiKernel_SectionAccordionHeader = atuiKernel_SectionAccordionContainer[
        i
    ].querySelector(".atuiKernel_SectionAccordionHeader");
    atuiKernel_SectionAccordionContent = atuiKernel_SectionAccordionContainer[
        i
    ].querySelector(".atuiKernel_SectionAccordionContent");
...
function atuiKernel_SectionAccordionDisplay(content) {
    const atuiKernel_SectionAccordionArrow =
        atuiKernel_SectionAccordionHeader.querySelector(
